// Source:  https://silvestreh.github.io/onScreen/index.html
// Source:  https://unpkg.com/onscreen@1.3.2/dist/on-screen.umd.js
// Source:  https://unpkg.com/onscreen@1.3.2/dist/on-screen.umd.min.js
// Version: 1.3.2
/**
	Funcionamento:
		var os = new OnScreen({ 
		...
		os.on('enter', 'cssSelector', function(el, event){});
		os.on('leave', 'cssSelector', function(el, event){});
		os.off('enter', 'cssSelector');
		os.destroy() | os.attach();
		OnScreen.check(cssSelector[, container])
	
	// Observação:
	// --> IMAGINACOM:
	//     --> Decidimos colocar suporte ao jQuery, embora o autor original não o fizesse.
	//     --> Esse suporte é apenas um atalho para as funções "os.".
	$('div').onScreen(cbEnter, cbLeave);
	$.onScreen.setTolerance(-100); --> Quantos pixels podem aparecer antes do evento ser disparado
	$.onScreen.setDebounce ( 100); --> Atualizar apenas quantos ms após a rolagem?
	$.onScreen.getOS();            --> Recupera o "OnScreen" objeto para manipulação direta
**/
...
(function($){
	...
	var _init   = function(){
		if(_inited)
			return;
		_inited = true;
		os = new OnScreen(_defOptions);
	};
	
	var _defOptions = {
		tolerance: 50, // 100px antes de aparecer ou depois de sumir
		debounce:   100, // só atualiza 100ms depois de terminar a rolagem
		container: window,
	};
	
	$.onScreen = {};
	$.onScreen.setTolerance = function(tolerance){
		if(_inited){
			os.options.tolerance = tolerance;
		}
		else{
			_defOptions.tolerance = tolerance;
		}
	};
	$.onScreen.setDebounce = function(debounce){
		if(_inited){
			os.options.debounce  = debounce;
		}
		else{
			_defOptions.debounce = debounce;
		}
	};
	$.onScreen.getOS = function(){
		return os;
	};
	
	$.fn.onScreen = function(cbEnter, cbLeave){
		_init();
  		if(isSelectorValid(this.selector)){
			if(cbEnter) os.on('enter', this.selector, function(el){ cbEnter.call(el); });
			if(cbLeave) os.on('leave', this.selector, function(el){ cbLeave.call(el); });
		}
		else{
			alert("jQuery.onScreen: Seletor inválido: ", this.selector);
		}
	};
	var isSelectorValid = (function(selector){
		// Check if is a valid CSS Selector
