...
class Parser
{
    ...
    protected function parseChunk()
    {
        ...
        if ($this->variable($name) &&
            ...
            $this->append(array(Type::T_ASSIGN, $name, $value, $assignmentFlags), $s);
    ...
    protected function value(&$out)
    {
        ...
        if ($this->literal('url(') && $this->match('data:([a-z]+)\/([a-z0-9.+-]+);base64,', $m, false)) {
            ...
            if ($this->literal(')')) {
                ...
                $out = array(Type::T_KEYWORD, $content);
    ...
    protected function stripAssignmentFlags(&$value)
    {
        ...
        for ($token = &$value; $token[0] === Type::T_LIST && ($s = count($token[2])); $token = &$lastNode) {
            ...
            while ($lastNode[0] === Type::T_KEYWORD && in_array($lastNode[1], array('!default', '!global'))) {
                array_pop($token[2]);
